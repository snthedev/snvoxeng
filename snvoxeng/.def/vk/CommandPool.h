/*
 * ====== CommandPool Builder Configuration ======
 *
 * Available macros:
 *   > Required value
 *   _RVAR(storetype, argtype, name)
 *     -> Builder::withName(argtype ...)
 *     -> const storetype& CommandPool::getName()
 *   
 ...
 *     -> Builder::addName(...)
 *     -> CommandPool::getName()
 *     -> CommandPool::getName(idx)
 *     -> CommandPool::getNameSize()
 *     -> CommandPool::getNameData()
 *
...
_RVAR(uint32_t, uint32_t, QueueFamilyIndex);
#endif

#ifdef _OVAR
/* ====== Optional variables ====== */
_OVAR(VkCommandPoolCreateFlags, VkCommandPoolCreateFlags, Flags);
#endif
