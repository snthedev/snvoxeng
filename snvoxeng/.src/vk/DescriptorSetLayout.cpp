...
// === DescriptorSetLayout : private ===
struct DescriptorSetLayout::data_t
{
	VkDescriptorSetLayoutCreateInfo vkCreateInfo{ .sType{ ::sn::voxeng::utils::vk::getSType<VkDescriptorSetLayoutCreateInfo>() } };
...
void DescriptorSetLayout::onCreate(data_t& data)
{
	auto result = data.pDevice->createDescriptorSetLayout(&data.vkCreateInfo, data.vkPAllocator, &data.vkHandle);
	snassert(result == VK_SUCCESS,
		"Failed to create VkDescriptorSetLayout", "Check Builder settings");
	...
	if (data.pDevice->getPhysicalDevice().getRegistry().getInstance().getDebugStream())
		*data.pDevice->getPhysicalDevice().getRegistry().getInstance().getDebugStream()
		<< "[trace]: DescriptorSetLayout 0x" << std::hex << this << std::dec << " created" << std::endl;
}
void DescriptorSetLayout::onDestroy(data_t& data) noexcept
{
	data.pDevice->destroyDescriptorSetLayout(data.vkHandle, data.vkPAllocator);

	if (data.pDevice->getPhysicalDevice().getRegistry().getInstance().getDebugStream())
		*data.pDevice->getPhysicalDevice().getRegistry().getInstance().getDebugStream()
		<< "[trace]: DescriptorSetLayout 0x" << std::hex << this << std::dec << " destroyed" << std::endl;
}

DescriptorSetLayout::DescriptorSetLayout(data_t*& pData)
	: m_pData(pData)
	, m_isView(false)
{
	pData = nullptr;
...
DescriptorSetLayout::operator VkDescriptorSetLayout() const noexcept { return m_pData->vkHandle; }

#define SNBCG_REQUIRED(store_t, arg_t, subdata, name, Name, return_policy, store_policy)\
DETAIL_##return_policy##_t(store_t) DescriptorSetLayout::get##Name() const noexcept { std::add_lvalue_reference_t<std::add_const_t<store_t>> val = m_pData->subdata name; return return_policy; }
#define SNBCG_OPTIONAL(store_t, arg_t, subdata, name, Name, return_policy, store_policy)\
DETAIL_##return_policy##_t(store_t) DescriptorSetLayout::get##Name() const noexcept { std::add_lvalue_reference_t<std::add_const_t<store_t>> val = m_pData->subdata name; return return_policy; }
#define SNBCG_REQUIRED_ADDITIVE(store_t, arg_t, args_t, subdata, name, Name, return_policy, store_policy, store_action)\
DETAIL_##return_policy##_t(store_t) DescriptorSetLayout::get##Name() const noexcept { std::add_lvalue_reference_t<std::add_const_t<store_t>> val = m_pData->subdata name; return return_policy; }
...
typedef DescriptorSetLayout::Builder Builder;

// === Builder : private ===
void Builder::finalize(data_t& data)
{
	data.vkCreateInfo.bindingCount = static_cast<uint32_t>(data.bindings.size());
	data.vkCreateInfo.pBindings = data.bindings.data();
}
